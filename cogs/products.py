...
class ProductCommands(commands.Cog):
    ...
    # -----------------------------
    # Utility: Forward image to storage channel
    # -----------------------------
    async def forward_attachment_to_storage_channel(self, guild, attachment):
        """Send the attachment to the configured storage channel. Returns CDN URL or None."""
        try:
            ...
            # If forwarding fails, fall back to original URL
            return None
    ...
    async def add(
        ...
        if image is None:
            return await interaction.followup.send(
                "❌ You must attach an image file for the product.",
                ephemeral=True,
            )
        ...
        # Save image to storage channel if available
        ...
        await interaction.followup.send(
            f"✅ Product **{name}** (ID {new_id}) added successfully!",
            ephemeral=True,
        )
    ...
    async def remove(self, interaction: discord.Interaction, product_id: int):
        ...
        await interaction.response.send_message(
            f"🗑️ Product **{product['name']}** removed.", ephemeral=True
        )
    ...
    async def list_products(self, interaction: discord.Interaction):
        ...
        if not products:
            return await interaction.response.send_message(
                "⚠️ No products available.", ephemeral=True
            )
